...
import com.example.demo.repository.SharedPostRepository;
...
public class PostServiceImpl implements PostService {
    ...
    @Override
    public Post createPost(String userId, String description, MultipartFile[] mediaFiles, boolean isVideo) throws IOException {

        try {
            Post post = new Post();
            post.setUserId(userId); // ✅ Set userId here
    ...
    @Override
    public Post updatePostDescription(String id, String description) {
    ...
    @Override
    public SharedPost sharePost(String postId, String sharingUserId, String description) {
        Optional<Post> optionalPost = postRepository.findById(postId);
        if (optionalPost.isPresent()) {
            Post originalPost = optionalPost.get();
            SharedPost sharedPost = new SharedPost();
            sharedPost.setSharedBy(sharingUserId);
            sharedPost.setOriginalPostId(postId);
            sharedPost.setDescription(description);

            // Copy media information from the original post
    ...
    public List<SharedPost> getSharedPostsByUserId(String userId) {
        return sharedPostRepository.findBySharedBy(userId);
    }
}
